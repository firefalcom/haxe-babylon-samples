...
var SPS = function(name,scene,options,ground,widthNb,heightNb,size,gravity,restitution,friction,speed) {
	this.boom = false;
	this.size = size;
	...
	this.ground = ground;
	BABYLON.SolidParticleSystem.call(this,"sps",scene,{ isPickable : true});
};
SPS.__name__ = true;
SPS.__super__ = BABYLON.SolidParticleSystem;
SPS.prototype = $extend(BABYLON.SolidParticleSystem.prototype,{
	initParticles: function() {
		var p = 0;
		var _g1 = 0;
		var _g = this.heightNb;
		while(_g1 < _g) {
			var j = _g1++;
			var _g3 = 0;
			var _g2 = this.widthNb;
			while(_g3 < _g2) {
				var i = _g3++;
				this.particles[p].position.x = i * this.size + this.vars.shiftx;
				this.particles[p].position.y = j * this.size + this.vars.shifty;
				this.particles[p].position.z = 0;
				this.particles[p].uvs.x = i * this.size / this.vars.totalWidth;
				this.particles[p].uvs.y = j * this.size / this.vars.totalHeight;
				this.particles[p].uvs.z = (i + 1) * this.size / this.vars.totalWidth;
				this.particles[p].uvs.w = (j + 1) * this.size / this.vars.totalHeight;
				this.particles[p]["rand"] = 1 / (1 + Math.random()) / 10;
				++p;
			}
		}
	}
	,updateParticle: function(p) {
		if(this.vars.justClicked) {
			p.position.subtractToRef(this.vars.target,this.vars.tmp);
			var len = this.vars.tmp.length();
			var scl = len < 0.001 ? 1.0 : this.vars.radius / len;
			this.vars.tmp.normalize();
			p.velocity.x += this.vars.tmp.x * scl * this.speed * (1 + Math.random() * .3);
			p.velocity.y += this.vars.tmp.y * scl * this.speed * (1 + Math.random() * .3);
			p.velocity.z += this.vars.tmp.z * scl * this.speed * (1 + Math.random() * .3);
			if(p.idx == this.nbParticles - 1) {
				this.vars.justClicked = false;
			}
		}
		if(this.boom && !this.vars.justClicked) {
			var tmp = this.ground.getHeightAtCoordinates(p.position.x,p.position.z);
			this.vars.minY = tmp + this.size;
			var tmp1 = -this.restitution * Reflect.field(p,"rand");
			this.vars.loss = tmp1 * 10;
			if(p.position.y < this.vars.minY) {
				this.ground.getNormalAtCoordinatesToRef(p.position.x,p.position.z,this.vars.norm);
				var tmp2 = 2 * (this.vars.norm.x * p.velocity.x + this.vars.norm.y * p.velocity.y + this.vars.norm.z * p.velocity.z);
				var tmp3 = this.vars.norm.lengthSquared();
				this.vars.sym = tmp2 / tmp3;
				p.velocity.x = this.vars.sym * this.vars.norm.x - p.velocity.x;
				p.velocity.z = this.vars.sym * this.vars.norm.z - p.velocity.z;
				p.velocity.y = this.vars.sym * this.vars.norm.y - p.velocity.y;
				p.velocity.x *= this.vars.loss;
				p.velocity.y *= this.vars.loss;
				p.velocity.z *= this.vars.loss;
			}
			...
			if(p.position.y < this.vars.minY && Math.abs(p.velocity.y) < 0.1 - this.gravity) {
				p.velocity.x *= this.friction;
				p.velocity.z *= this.friction;
				p.position.y = this.vars.minY;
				p.velocity.y = 0;
			}
		}
		return p;
	}
	,afterUpdateParticles: function(start,stop,update) {
...
var Boom = function(width,height) {
...
Boom.__super__ = Launcher;
Boom.prototype = $extend(Launcher.prototype,{
	createScene: function() {
		var size = 10;
		var gravity = -0.07;
		var restitution = 0.9;
		...
		var widthNb = 30;
		var heightNb = 20;
		var radius = size * heightNb / 12;
		var speed = radius * 1.2;
		var subdivisions = 50;
		var width = 1000;
		var height = 1000;
		...
		var diffuseTexture = new BABYLON.Texture("../../assets/ground.jpg",scene);
		...
		var diffuseTexture1 = new BABYLON.Texture("../../assets/ground.jpg",scene);
		groundMaterial2.diffuseTexture = diffuseTexture1;
		diffuseTexture1.uScale = 50;
		diffuseTexture1.vScale = 50;
		...
		sps.vars.minY = 0.0;
		sps.vars.norm = BABYLON.Vector3.Zero();
		sps.vars.sym = 0.0;
		sps.vars.loss = 0.0;
		...
		scene.onPointerDown = function(evt,pickResult,e) {
			var faceId = pickResult.faceId;
			if(faceId == -1) {
				return;
			}
			var idx = sps.pickedParticles[faceId | 0].idx;
			var p = sps.particles[idx | 0];
			sps.boom = true;
