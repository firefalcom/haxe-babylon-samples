...
class AssetManager {
    ...
    static function main() {
        js.Browser.document.addEventListener("DOMContentLoaded", run, false);
    }

    static function run() {
        if(BABYLON.Engine.isSupported()) {
            BABYLON.Engine.ShadersRepository = "/Babylon/Shaders/";
            var canvas = js.Browser.document.getElementById("renderCanvas");
            var engine = new BABYLON.Engine(cast canvas, false);
            // Resize
            ...
            assetsManager.onTaskError = function(task) {
                trace("error while loading " + task.name);
            }
